...
describe('AuthController', () => {
  ...
  describe('signup', () => {
    ...
    it('DTO validation - invalid phone number ', async () => {
      ...
      await target.transform(pc, metadataPC).catch((err) => {
    ...
    it('DTO validation - contactInformation empty ', async () => {
      ...
      await target.transform(pc, metadataPC).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'contactInformation should not be empty',
        ]);
      });
    });
    ...
    it('DTO validation - campaignName empty ', async () => {
      ...
      await target.transform(pc, metadataPC).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'campaignName should not be empty',
        ]);
      });
    });
    it('DTO validation - studyField empty ', async () => {
      ...
      await target.transform(pc, metadataPC).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'studyField should not be empty',
        ]);
      });
    });
    it('DTO validation - politicalParty empty ', async () => {
      ...
      await target.transform(pc, metadataPC).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'politicalParty should not be empty',
        ]);
      });
    });
    it('DTO validation - biography empty ', async () => {
      ...
      await target.transform(pc, metadataPC).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'biography should not be empty',
        ]);
      });
    });
    it('DTO validation - occupation empty ', async () => {
      citizen.occupation = '';
      await target.transform(citizen, metadataCitizen).catch((err) => {
        expect(err.getResponse().message).toEqual([
    ...
    it('DTO validation - placeOfBirth empty ', async () => {
      ...
      await target.transform(citizen, metadataCitizen).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'placeOfBirth should not be empty',
        ]);
      });
    });
    ...
    it('DTO validation - position empty ', async () => {
      mp.position = '';
      await target.transform(mp, metadataMP).catch((err) => {
        expect(err.getResponse().message).toEqual([
    ...
    it('DTO validation - relatedParty empty ', async () => {
      ...
      await target.transform(mp, metadataMP).catch((err) => {
        expect(err.getResponse().message).toEqual([
          'relatedParty should not be empty',
        ]);
      });
    });
    it('DTO validation - biography empty ', async () => {
    ...
    it('signup for citizen ', async () => {
      ...
      jest
        .spyOn(authService, 'signUpCitizen')
        .mockImplementationOnce(async () =>
          responseMock.status(200),
        );
      await controller.signUpCitizen(citizen, responseMock);

      expect(responseMock.status).toHaveBeenCalledWith(200);
    });
    it('signup for pc ', async () => {
      const responseMock = {
        status: jest.fn().mockReturnThis(),
        json: jest.fn(),
      } as unknown as any;
      ...
      jest
        .spyOn(authService, 'signUpPc')
        .mockImplementationOnce(async () =>
          responseMock.status(200),
        );
      await controller.signUpPc(pc, responseMock);

      expect(responseMock.status).toHaveBeenCalledWith(200);
    });
    it('signup for mp ', async () => {
      const responseMock = {
        status: jest.fn().mockReturnThis(),
        json: jest.fn(),
      } as unknown as any;
      ...
      jest
        .spyOn(authService, 'signUpMp')
        .mockImplementationOnce(async () =>
          responseMock.status(200),
        );
      await controller.signUpMp(mp, responseMock);
  ...
  describe('login', () => {
    it('it should return login successful with status 200', async () => {
      ...
      jest
        ...
        .mockImplementationOnce(async () =>
          responseMock.status(200).json({ message: 'Login successful' }),
        );
    ...
    it('it should return invalid credentials with status code ', async () => {
      ...
      jest
        .spyOn(authService, 'loginUser')
        .mockImplementationOnce(async () =>
          responseMock.status(401).json({ message: 'Invalid Credentials.' }),
        );
      await controller.loginUser(userData, responseMock);
